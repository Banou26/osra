...
const isTransferBox = (value: unknown): value is { inner: unknown } =>
  Boolean(
    value
    && typeof value === 'object'
 ...
 *      when the walker is inside a transfer box — i.e. when the user
 *      explicitly opted into move semantics at the send site via transfer().
 *
...
export const getTransferableObjects = (value: unknown): Transferable[] => {
  ...
  const recurse = (value: unknown, inTransferBox: boolean): void => {
    ...
    if (isTransferBox(value)) {
      // Flip into transfer mode — every Transferable found below this point
      // on this branch of the walk gets added to the transfer list.
      recurse(value.inner, true)
      return
    }
