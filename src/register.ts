...
import { makeAllocator, makeNumberAllocator, replaceIncomingProxiedTypes, replaceOutgoingProxiedTypes } from './utils'
...
export const expose = async <T extends StructuredCloneTransferableType, T2 extends StructuredCloneTransferableType>(
  ...
  const uuid = await
    'crypto' in globalThis && 'randomUUID' in globalThis.crypto ? globalThis.crypto.randomUUID()
    : 'require' in globalThis ? require('crypto').randomUUID()
      ...
      remote(
        {
          ...
          key,
          type: 'init',
          data: replaceOutgoingProxiedTypes(value, getContext())
        },
        []
      )
      return
