...
export type BoxedIdentity = BoxBaseType<typeof type> & {
  id: string
  inner?: Capable
}
...
export const box = <T extends IdentityWrapper, TContext extends RevivableContext>(
  value: T,
  context: TContext,
): BoxedIdentity => {
  ...
  const innerBox = recursiveBox(inner as Capable, context) as Capable
  if (key !== undefined) {
...
export const revive = <T extends BoxedIdentity, TContext extends RevivableContext>(
  ...
  const revived = recursiveRevive(value.inner as Capable, context)
