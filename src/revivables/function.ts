...
/**
 * FinalizationRegistry for closing the call-port when the revived function
 * proxy is garbage collected. Identity-layer cleanup (evicting the id cache
 * and sending the `revivable-drop` message) is handled by the generic
 * revivableCleanupRegistry in `src/revivables/index.ts` — this FR only
 * handles the function-specific port.
 */
...
export const revive = <T extends BoxedFunction, T2 extends RevivableContext>(
  ...
  // Register for port cleanup when the proxy is garbage collected.
